...
sem = asyncio.Semaphore(50)  # ← 50 threads
    ...
    async with sem:
        ...
        try:
            async with session.get(url, timeout=5) as resp:
                ...
                print(f"  • Title: {BeautifulSoup(await resp.text(), 'html.parser').title.string.strip()}")
...
async def main():
    ...
    print(f"\nCompleted with 50 threads in {round(time.time() - start, 2)} s")
